...
const DirectTruckViewer: React.FC<Container3DProps> = ({ container, placedItems, showWeightHeatmap, cameraView = 'iso' }) => {
  ...
  useEffect(() => {
    ...
    if (showWeightHeatmap && placedItems.length > 0) {
      ...
      cog.position.set(((weighted.x - container.width / 2) / 100) * lateralDisplayScale, weighted.y / 100 + cargoLift, (weighted.z - container.length / 2) / 100 + cargoDeckOffsetZ);
      scene.add(cog);
    }
        ...
        model.traverse((object) => {
          ...
          materials.forEach((material) => {
            ...
            if ('color' in material && material.color instanceof THREE.Color) {
              material.color.lerp(new THREE.Color('#22d3ee'), 0.86);
              material.color.lerp(new THREE.Color('#ffffff'), 0.68);
            }
            if ('emissive' in material && material.emissive instanceof THREE.Color) {
              material.emissive.set('#14b8a6');
